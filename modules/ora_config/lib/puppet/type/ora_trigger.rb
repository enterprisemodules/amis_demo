...
Puppet::Type.newtype(:ora_trigger) do
  include EasyType
  ...
  to_get_raw_resources do
    sql_on_all_database_sids "select distinct owner, name  from dba_source where type = 'TRIGGER'"
  end
  ...
  on_destroy do
    [:sql, "drop trigger #{owner}.#{trigger_name}", {:sid => sid}]
  end
  ...
  map_title_to_sid(:owner, :trigger_name) { /(.+)\.(.+)/ }
