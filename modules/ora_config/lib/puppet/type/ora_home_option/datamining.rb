...
newproperty(:datamining) do
  include EasyType
  include Puppet_X::EnterpriseModules::Oracle::Settings

  desc <<-EOD

      This resource allows you to manage(enable/disable) the Data Mining RDBMS Files option in an Oracle database home.
  ...
  on_modify do
    ...
    command = "su - #{os_user} -c \"export ORACLE_HOME=#{resource.name}; #{resource.name}/bin/chopt #{flag} dm\""
