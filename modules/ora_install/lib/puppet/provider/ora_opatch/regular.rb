...
Puppet::Type.type(:ora_opatch).provide(:regular, :parent => :base) do
  ...
  def apply_patch(source_dir, command_builder)
    ocmrf = resource.ocmrf_file.nil? ? '' : " -ocmrf #{resource.ocmrf_file}"
    command_builder.add "apply -silent #{ocmrf} -oh #{oracle_product_home_dir} #{source_dir} -local", :uid => os_user
  end

  def remove_patch(source_dir, command_builder)
    command_builder.add "rollback -id #{patch_id} -silent -oh #{oracle_product_home_dir} -local", :uid => os_user
  end
end
