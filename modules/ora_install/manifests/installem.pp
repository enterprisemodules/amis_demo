# == Class: ora_install::installem
#
#
define ora_install::installem(
  $version                       = '12.1.0.5',
  $file                          = undef,
...
)
{
  ...
  if $found == undef {
    ...
    if ( $found ) {
      ...
      notify {"ora_install::installem ${oracle_home_dir} does not exists":}
      $continue = true
    }
  }
  ...
  if ( $continue ) {
    ...
    ora_install::utils::dborainst{"em orainst ${version}":
      ora_inventory_dir => $ora_inventory,
      os_group          => $group,
    }
    ...
    if ! defined(File["${download_dir}/em_install_${version}.rsp"]) {
      file { "${download_dir}/em_install_${version}.rsp":
        ...
        content => template("ora_install/em_install_${version}.rsp.erb"),
        mode    => '0775',
        owner   => $user,
        group   => $group,
        require => [Ora_install::Utils::Dborainst["em orainst ${version}"],
                    Db_directory_structure["oracle em structure ${version}"],],
      }
    }
    if ! defined(File["${download_dir}/em_install_static_${version}.ini"]) {
      file { "${download_dir}/em_install_static_${version}.ini":
        ...
        content => template("ora_install/em_install_static_${version}.ini.erb"),
        mode    => '0775',
        owner   => $user,
        group   => $group,
        require => [Ora_install::Utils::Dborainst["em orainst ${version}"],
